...
###bild laden
filename = os.path.join("X:\\user\\Lana", 'Bild7_20190403.bmp')
bild = io.imread(r"X:\user\Lana\Bild7_20190403.bmp")
print(bild.shape)
plt.gray()
...
####maxima suchen
maxima = argrelmax(lineprofile, order=6)[0]
maxliste = list(maxima)

slices = []
for i in range(len(maxliste) - 1):
    von = maxliste[i]
    bis = maxliste[i + 1]
    slice = (von, bis)
    slices.append(slice)
...
min = maxliste[1:]
abstand = []
for x in range(len(maxliste) - 1):
    abstand1 = min[x] - maxliste[x]
